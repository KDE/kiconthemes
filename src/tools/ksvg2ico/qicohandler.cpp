...
{
    if (icoAttrib.ncolors > 0) {
        // set color table
        readColorTable(image);
    } else if (icoAttrib.nbits == 16) {
        // don't support RGB values for 15/16 bpp
        image = QImage();
    }
}
...
bool QtIcoHandler::write(const QVector<QImage> &images)
{
    QIODevice *device = QImageIOHandler::device();
